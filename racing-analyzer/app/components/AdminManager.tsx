...
import LayoutsModal from './admin/LayoutsModal';
...
/**
 * Shared admin management UI (users + tracks + per-track layouts). Rendered both
 * in the dashboard's Admin tab (dark) and the standalone /admin page. All
 * mutations go through AuthContext.apiFetch so the CSRF token is always sent.
 */
export default function AdminManager({ isDarkMode = false }: { isDarkMode?: boolean }) {
  ...
  const [layoutsFor, setLayoutsFor] = useState<Track | null>(null);
  ...
  const handleDeleteTrack = async (trackId: number) => {
    ...
    try {
      const response = await apiFetch(`/api/admin/tracks/${trackId}`, { method: 'DELETE' });
  ...
  return (
    <div className="p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="flex items-center">
                  <input type="checkbox" checked={trackForm.is_active}
                    onChange={(e) => setTrackForm({ ...trackForm, is_active: e.target.checked })} className="mr-2" />
      ...
      {/* Per-track layouts */}
      {layoutsFor && <LayoutsModal track={layoutsFor} onClose={() => setLayoutsFor(null)} />}
    </div>
  );
}
